...
export async function run(provider: NetworkProvider) {
    ...
    ui.write(
        'Enter withdraw fee. It has to be at least 0.2 TON. For more info check out https://www.notion.so/DAOWithSplitter-contract-description-1b3f8e6f8ab5805cb76fc1f15890975c?pvs=4#1b9f8e6f8ab580d8bb2ccc74f6afaa3e',
    );
    const withdrawalFeeInput = await ui.input('Withdrawal fee in full TON (at least 0.2 TON):');

    // Validate withdrawal fee is a valid number
    const withdrawalFee = parseFloat(withdrawalFeeInput);
    if (isNaN(withdrawalFee) || withdrawalFee < 0.2) {
        ui.write('Invalid withdrawal fee. Must be at least 0.2 TON.');
        return;
    }
    ...
    ui.write(`DAO will be deployed at ${dao.address}`);
